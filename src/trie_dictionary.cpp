  ...
  std::string seq, std::size_t max_cost) noexcept
{
  ...
  for (auto child : trie_.root_->children)
  {
    ...
    search_recursive(trie_.root_->children[Trie::charToIndex(letter)], 
        letter, seq, curr_row, results, max_cost);
  }
  
  return results;
}

void TrieDictionary::search_recursive(Trie::Node* node, 
    char letter,
    std::string word, 
    ...
    std::size_t max_cost)
{
  ...
  if (*std::min_element(curr_row.begin(), curr_row.end()) <= max_cost)
  {
    for (auto child : node->children)
    {
      ...
      auto letter = child->chr;
      search_recursive(node->children[Trie::charToIndex(letter)], 
          letter, word, curr_row, results, max_cost);
    }
  }
}
